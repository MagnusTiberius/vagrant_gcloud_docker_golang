...
Vagrant.configure("2") do |config|
  ...
  #config.vm.provision "shell", inline: $setup_golang_git
  config.vm.provision "shell", inline: $setup_gcloud
  ...
  config.vm.provision "shell", inline: <<-SHELL
    ...
    sudo cp protoc-3.4.0-linux-x86_64/bin/protoc /usr/local/bin/
  SHELL
